...
const R16_FORMATS: [(GpuTextureFormat, GpuShaderIoScalarClass); 3] = [
    (GpuTextureFormat::R16Uint, GpuShaderIoScalarClass::Uint),
    (GpuTextureFormat::R16Sint, GpuShaderIoScalarClass::Sint),
    (GpuTextureFormat::R16Float, GpuShaderIoScalarClass::Float),
];

const RG16_FORMATS: [(GpuTextureFormat, GpuShaderIoScalarClass); 3] = [
    (GpuTextureFormat::Rg16Uint, GpuShaderIoScalarClass::Uint),
    (GpuTextureFormat::Rg16Sint, GpuShaderIoScalarClass::Sint),
    (GpuTextureFormat::Rg16Float, GpuShaderIoScalarClass::Float),
];
...
#[test]
fn r16_public_metadata_and_copy_geometry_are_exact() {
    assert_public_metadata(&R16_FORMATS, 2);
}

#[test]
fn rg16_public_metadata_and_copy_geometry_are_exact() {
    assert_public_metadata(&RG16_FORMATS, 4);
}
...
#[test]
fn r16_structural_normalization_preserves_observed_roles() {
    assert_structural_normalization_preserves_observed_roles(&R16_FORMATS, 2);
}

#[test]
fn rg16_structural_normalization_preserves_observed_roles() {
    assert_structural_normalization_preserves_observed_roles(&RG16_FORMATS, 4);
}
...
fn admitted_sampled_class(
    ...
    let identity = GpuProgramSourceIdentity::new(
        owner,
        GpuProgramSourceKey::new("r1.r-rg16.sampled").unwrap(),
    ...
    let source = registry
        .admit_wgsl(
            identity,
            &source_text,
            GpuProgramSourceProvenance::new("r1-r-rg16-format-test", None).unwrap(),
        )
        .unwrap();
...
#[test]
fn canonical_wgsl_derives_r_rg16_sampled_scalar_classes_without_storage_formats() {
    assert_eq!(
...
#[test]
fn r_rg16_storage_texels_fail_closed_without_normalized_tier1() {
    for storage_format in [
        ...
        let identity = GpuProgramSourceIdentity::new(
            owner,
            GpuProgramSourceKey::new("r1.r-rg16.storage-tier").unwrap(),
            GpuProgramSourceRevision::try_from_raw(1).unwrap(),
        );
        let mut registry = GpuProgramSourceRegistry::new(2, 16 * 1024).unwrap();
        let source = registry
...
#[test]
fn r16_fragment_io_and_integer_blending_follow_scalar_class() {
    assert_fragment_io_and_integer_blending_follow_scalar_class(&R16_FORMATS, 1);
}

#[test]
fn rg16_fragment_io_and_integer_blending_follow_scalar_class() {
    assert_fragment_io_and_integer_blending_follow_scalar_class(&RG16_FORMATS, 2);
}
...
#[test]
fn r16_prepared_texture_uses_two_byte_rows() {
    assert_prepared_texture_rows(&R16_FORMATS, 2, "R16");
}

#[test]
fn rg16_prepared_texture_uses_four_byte_rows() {
    assert_prepared_texture_rows(&RG16_FORMATS, 4, "RG16");
}
...
fn native_copy_family_qualification_fails_closed_without_gpu() {
    for (family, full) in [("R16", 3), ("RG16", 3)] {
        assert!(
...
fn r16_native_copy_round_trips_per_observed_format() {
    let formats = R16_FORMATS.map(|entry| entry.0);
    let exercised = run_native_copy_family(&formats, &[127, 128], 2, "R16");
    println!(
        "R16 native copy proofs: {exercised}/3 formats exercised; all-skipped is NOT qualified"
    );
    assert_native_copy_family_qualified(exercised, "R16");
}

#[test]
#[ignore = "requires a Vulkan software adapter; executed by RunenGPU native Conformance CI"]
fn rg16_native_copy_round_trips_per_observed_format() {
    let formats = RG16_FORMATS.map(|entry| entry.0);
    let exercised = run_native_copy_family(&formats, &[63, 64], 4, "RG16");
    println!(
        "RG16 native copy proofs: {exercised}/3 formats exercised; all-skipped is NOT qualified"
    );
    assert_native_copy_family_qualified(exercised, "RG16");
}
